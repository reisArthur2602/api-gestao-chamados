...
ClientRoutes.delete("/:id", isAuthenticated, async (request, response) => {
  const params = z.object({ id: z.string().min(1) }).parse(request.params);

  const client = await clientController.delete(params.id);
  return response.status(StatusCodes.OK).json(client);
});

ClientRoutes.put("/", isAuthenticated, async (request, response) => {
  const body = z
    .object({
